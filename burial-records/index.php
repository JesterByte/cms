<?php 
  ...
  require_once "../content/burial-records.php";

  if (!isset($_SESSION["user_id"])) {
    serverRedirect("../");
  }

  function displayBurialRecordsTable($burialRecordsTable) {
    foreach ($burialRecordsTable as $burialRecordsRow) {
        startRow();
        rowData($burialRecordsRow["full_name"]);
        rowData(displayDate($burialRecordsRow["birth_date"]));
        rowData(displayDate($burialRecordsRow["death_date"]));
        rowData(displayDate($burialRecordsRow["burial_date"]));
        rowData(displayPhaseLocation($burialRecordsRow["location"]));
        rowData(displayDateTime($burialRecordsRow["created_at"]));
        endRow();
    }    
  }
?>
      ...
      <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
            ...
            <div class="table-responsive rounded shadow">
                <table id="burialRecordsTable" class="table table-striped table-hover table-bordered text-center" style="width:100%">
                <thead>
                    <tr>
                    <th>Name</th>
                    <th>Date of Birth</th>
                ...
                <tbody>
                    <?= displayBurialRecordsTable($burialRecordsTable); ?>
                </tbody>
                </table>
            </div>
        </main>
