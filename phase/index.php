<?php 
  ...
  function displayPhasePricingTable($phasePricingTable) {
    foreach ($phasePricingTable as $phasePricingRow) {
      startRow();
      rowData($phasePricingRow["phase"]);
      rowData($phasePricingRow["lot_type"]);
      rowData($phasePricingRow["number_of_lots"]);
      ...
      rowData(formatToPeso($phasePricingRow["monthly_amortization_2yrs_10_interest"]));
      rowData(formatToPeso($phasePricingRow["monthly_amortization_3yrs_15_interest"]));
      rowData(formatToPeso($phasePricingRow["monthly_amortization_4yrs_20_interest"]));
      rowData(formatToPeso($phasePricingRow["monthly_amortization_5yrs_25_interest"]));
      endRow();
    }
  }
...
<html lang="en" data-bs-theme="auto">
  ...
  <body>
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
          ...
          <div class="table-responsive rounded shadow">
            <table id="phasePricingTable" class="table table-striped table-hover table-bordered text-center" style="width:100%">
              ...
              <tbody>
                <?= displayPhasePricingTable($phasePricingTable); ?>
              </tbody>
            </table>
          </div>
        </main>
    ...
    <script>
      const priceUpdated = <?= echoSessionToast("phase_price_updated"); ?>;
    ...
    <script>
      modalAutofocus("updatePhasePricingModal", "phaseSelect");
    </script>
